...
guardian-qa-smoke-all-help: ## Print master smoke-all suite list and env knobs
	...
	@echo "     GUARDIAN_EVAL_PROMPT_COOLDOWN_SECONDS  (default 60s on cpu-16gb / unset; 0 disables)"
	@echo "     GUARDIAN_EVAL_TIMEOUT_SECONDS=…           (per-prompt HTTP timeout; +30m buffer when unset)"
...
laptop-up-fresh: dev-stack-fresh ## Wipe DB + bootstrap + laptop-up (destructive clean slate before Phase 212 etc.)
	@$(MAKE) laptop-up
