...
**Current focus:** **Pi and edge hookup** — the API and worker already queue `pending_command` as JSON (`command`, and usually `schedule_id`; program fires also include `program_id`). The stock [`pi_client/gr33n_client.py`](pi_client/gr33n_client.py) reads dict-shaped commands and posts feedback to `POST /actuators/{id}/events` with `API_KEY` auth; next work is to **exercise that path against a real farm** (device ↔ actuator wiring in `config.yaml`, TLS/base URL, clock skew), extend Pi feedback payloads if we want `rule_id` / `program_id` mirrored on Pi-reported events (today the handler only accepts `triggered_by_schedule_id`), and keep [`docs/mqtt-edge-operator-playbook.md`](docs/mqtt-edge-operator-playbook.md) in sync. **Phase 22** (program-tick worker + final `metadata.steps` backfill sweep) is **done** on `main` — see [Roadmap Status](#roadmap-status). Phase 14 field-network & commons work is largely complete. Key playbooks: [`docs/workflow-guide.md`](docs/workflow-guide.md), [`docs/mqtt-edge-operator-playbook.md`](docs/mqtt-edge-operator-playbook.md), [`docs/insert-commons-pipeline-runbook.md`](docs/insert-commons-pipeline-runbook.md), [`docs/insert-commons-receiver-playbook.md`](docs/insert-commons-receiver-playbook.md), [`docs/notifications-operator-playbook.md`](docs/notifications-operator-playbook.md), [`docs/domain-modules-operator-playbook.md`](docs/domain-modules-operator-playbook.md), [`docs/mobile-distribution.md`](docs/mobile-distribution.md), [`docs/audit-events-operator-playbook.md`](docs/audit-events-operator-playbook.md), [`docs/terminology-guideline.md`](docs/terminology-guideline.md), [`docs/phase-13-operator-documentation.md`](docs/phase-13-operator-documentation.md), [`docs/phase-14-operator-documentation.md`](docs/phase-14-operator-documentation.md).
