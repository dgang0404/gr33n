...
**Current focus:** **Phase 23 — stabilization sprint** ([plan](docs/plans/phase_23_stabilization_sprint.plan.md)): tighten CI (`go test`, `make audit-openapi`), smoke reliability, operator docs vs shipped behavior, and small bugfixes only — **no new features** until the exit checklist is done. Intended sequence next: **Phase 21** crop-cycle analytics ([plan](docs/plans/phase_21_crop_cycle_analytics.plan.md)), then **Phase 24 — RAG retrieval** ([plan](docs/plans/phase_24_rag_retrieval_system.plan.md)) on **Phase 20.95 RAG-prep** (schema only today). **Pi / edge field validation** remains a parallel activity when you are ready: real Pi or bridge against your API with `PI_API_KEY`, `config.yaml` device ↔ actuator wiring, then confirm readings and **`actuator_events`** in Postgres/UI ([`pi_client/gr33n_client.py`](pi_client/gr33n_client.py), [`cmd/api/smoke_pi_contract_test.go`](cmd/api/smoke_pi_contract_test.go)). **Phase 22** is done on `main`; **Phase 21** (full cycle analytics per [plan](docs/plans/phase_21_crop_cycle_analytics.plan.md)) is still planned — only **`GET /crop-cycles/{id}/cost-summary`** (20.7) exists today. Key playbooks: [`docs/workflow-guide.md`](docs/workflow-guide.md), [`docs/mqtt-edge-operator-playbook.md`](docs/mqtt-edge-operator-playbook.md), [`docs/insert-commons-pipeline-runbook.md`](docs/insert-commons-pipeline-runbook.md), [`docs/insert-commons-receiver-playbook.md`](docs/insert-commons-receiver-playbook.md), [`docs/notifications-operator-playbook.md`](docs/notifications-operator-playbook.md), [`docs/domain-modules-operator-playbook.md`](docs/domain-modules-operator-playbook.md), [`docs/mobile-distribution.md`](docs/mobile-distribution.md), [`docs/audit-events-operator-playbook.md`](docs/audit-events-operator-playbook.md), [`docs/terminology-guideline.md`](docs/terminology-guideline.md), [`docs/phase-13-operator-documentation.md`](docs/phase-13-operator-documentation.md), [`docs/phase-14-operator-documentation.md`](docs/phase-14-operator-documentation.md).
...
| **23** | **Stabilization sprint** — CI gates, smoke hardening, OpenAPI↔routes parity, edge/Pi docs, worker observability; **no new features** until exit criteria | 🔄 In progress (plan) | [plan](docs/plans/phase_23_stabilization_sprint.plan.md) |
| **24** | **RAG retrieval system** — embeddings + farm-scoped retrieval API (+ optional LLM synthesis); **after Phase 23 + 21** (recommended); builds on [20.95 RAG-prep](docs/plans/phase_20_95_rag_prep_and_housekeeping.plan.md) | ⏳ Planned | [plan](docs/plans/phase_24_rag_retrieval_system.plan.md) |

**In flight / next up** (priority order):

- [ ] **Phase 23 — stabilization** — [`docs/plans/phase_23_stabilization_sprint.plan.md`](docs/plans/phase_23_stabilization_sprint.plan.md): tests + `make audit-openapi` + docs + small fixes only; exit checklist then **Phase 21** (analytics) before **Phase 24** (RAG).
- [ ] **Phase 21 — crop cycle analytics** — [`docs/plans/phase_21_crop_cycle_analytics.plan.md`](docs/plans/phase_21_crop_cycle_analytics.plan.md): summary / compare / UI / CSV (distinct from 20.7 `cost-summary` only).
- [ ] **Phase 24 — RAG system** — [`docs/plans/phase_24_rag_retrieval_system.plan.md`](docs/plans/phase_24_rag_retrieval_system.plan.md): real retrieval (vectors + API + consent boundaries); **not** the same as 20.95 schema prep alone; **after** 21 is sensible so embeddings can cite richer cycle metrics.
...
- [ ] Phase 23 — stabilization sprint (CI, smoke, OpenAPI parity, docs, small fixes) — [plan](docs/plans/phase_23_stabilization_sprint.plan.md)
- [ ] Phase 24 — RAG retrieval system (vectors + farm-scoped API; after 23) — [plan](docs/plans/phase_24_rag_retrieval_system.plan.md)
