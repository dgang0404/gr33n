...
func registerRoutes(mux *http.ServeMux, pool *pgxpool.Pool, worker *automationworker.Worker, pushDispatch *pushnotify.Dispatcher, adminUser string, adminHash []byte, hashFilePath string, fileStore filestorage.Store, fileCfg filestorage.Config, adminBindUserID uuid.UUID, adminBindEmail string, aiCfg ai.Config) {
	...
	mux.Handle("POST /v1/chat/proposals/suggest-empty-zone", jwt(http.HandlerFunc(aichat.PostSuggestEmptyZoneProposal)))
	mux.Handle("POST /v1/chat/proposals/seed-pending", jwt(http.HandlerFunc(aichat.PostSeedPendingProposal)))
	mux.Handle("POST /v1/chat/proposals/{id}/dismiss", jwt(http.HandlerFunc(aichat.PostDismissProposal)))
	...
	mux.Handle("GET /v1/chat/sessions/{session_id}/turns/{turn_index}/debug", jwt(http.HandlerFunc(aichat.GetTurnDebug)))
	...
	mux.Handle("GET /farms/{id}/crop-analytics/recipe-outcomes", jwt(http.HandlerFunc(cropcycle.RecipeOutcomes)))
	...
	mux.Handle("DELETE /farms/{id}/crop-profiles/{crop_key}", jwt(http.HandlerFunc(cropProfiles.DeleteByCropKey)))
	mux.Handle("GET /farms/{id}/crop-library/picker", jwt(http.HandlerFunc(cropProfiles.Picker)))
	...
	mux.Handle("POST /farms/{id}/layout-background", jwt(http.HandlerFunc(files.UploadFarmLayoutBackground)))
	mux.Handle("GET /farms/{id}/layout-background", jwt(http.HandlerFunc(files.GetFarmLayoutBackground)))
	mux.Handle("DELETE /farms/{id}/layout-background", jwt(http.HandlerFunc(files.DeleteFarmLayoutBackground)))
