-- =============================================================================
-- Phase 35 OC-35A — bootstrap lighting_programs for jadam_indoor_photoperiod_v1
--
-- New farms that apply the JADAM indoor template now get two first-class
-- lighting_programs (Veg 18/6 and Flower 12/12) with properly wired ON/OFF
-- schedules and executable_actions, matching the Phase 35 handler pattern.
--
-- Legacy farms that already have 'Light ON/OFF 18/6 Veg' orphan schedules are
-- unaffected; those schedules are inactive (is_active=FALSE) and coexist until
-- the operator upgrades via POST /farms/{id}/lighting-programs/from-preset.
--
-- Idempotent: all inserts use NOT EXISTS / ON CONFLICT guards.
-- =============================================================================

-- ─── helper: create lighting_program + paired schedules + actions ─────────────
-- Called from _bootstrap_jadam_indoor_photoperiod_v1 at the end; also safe to
-- call from other templates that have Veg/Flower zones and grow-light actuators.

CREATE OR REPLACE FUNCTION gr33ncore._bootstrap_phase35_lighting_block(
    p_farm_id BIGINT,
    p_tz      TEXT
)
RETURNS void
...
BEGIN
    -- ── resolve zones ───────────────────────────────────────────────────────
    SELECT id INTO v_veg_zone
    FROM gr33ncore.zones
    WHERE farm_id = p_farm_id AND name = 'Veg Room' AND deleted_at IS NULL
    LIMIT 1;

    SELECT id INTO v_flower_zone
    FROM gr33ncore.zones
    WHERE farm_id = p_farm_id AND name = 'Flower Room' AND deleted_at IS NULL
    LIMIT 1;

    -- ── devices (Pi relay controllers for grow lights) ──────────────────────
    INSERT INTO gr33ncore.devices
        (farm_id, zone_id, name, device_uid, device_type, status, config)
    SELECT
        p_farm_id,
        v_veg_zone,
        'Veg Room Light Relay',
        'bootstrap-veg-light-' || p_farm_id::TEXT,
        'raspberry_pi',
        'unknown'::gr33ncore.device_status_enum,
        '{"simulation": true}'::jsonb
    WHERE v_veg_zone IS NOT NULL
      AND NOT EXISTS (
          SELECT 1 FROM gr33ncore.devices d
          WHERE d.farm_id = p_farm_id
            AND d.device_uid = 'bootstrap-veg-light-' || p_farm_id::TEXT
      );
    ...
    SELECT
        p_farm_id,
        v_flower_zone,
        'Flower Room Light Relay',
        'bootstrap-flower-light-' || p_farm_id::TEXT,
        'raspberry_pi',
        'unknown'::gr33ncore.device_status_enum,
        '{"simulation": true}'::jsonb
    WHERE v_flower_zone IS NOT NULL
      AND NOT EXISTS (
          SELECT 1 FROM gr33ncore.devices d
          WHERE d.farm_id = p_farm_id
            AND d.device_uid = 'bootstrap-flower-light-' || p_farm_id::TEXT
      );

    SELECT d.id INTO v_dev_veg
    FROM gr33ncore.devices d
    WHERE d.farm_id = p_farm_id
      AND d.device_uid = 'bootstrap-veg-light-' || p_farm_id::TEXT
    LIMIT 1;

    SELECT d.id INTO v_dev_flower
    FROM gr33ncore.devices d
    WHERE d.farm_id = p_farm_id
      AND d.device_uid = 'bootstrap-flower-light-' || p_farm_id::TEXT
    LIMIT 1;
    ...
    INSERT INTO gr33ncore.actuators
        (device_id, farm_id, zone_id, name, actuator_type,
         hardware_identifier, current_state_text, config)
    SELECT
        v_dev_veg,
        p_farm_id,
        v_veg_zone,
    ...
    WHERE v_veg_zone IS NOT NULL
      AND NOT EXISTS (
          ...
          WHERE a.farm_id = p_farm_id
            AND a.name = 'Veg Room Grow Light'
    ...
    WHERE v_flower_zone IS NOT NULL
      AND NOT EXISTS (
          SELECT 1 FROM gr33ncore.actuators a
          WHERE a.farm_id = p_farm_id
            AND a.name = 'Flower Room Grow Light'
            AND a.deleted_at IS NULL
      );

    SELECT id INTO v_act_veg
    FROM gr33ncore.actuators
    WHERE farm_id = p_farm_id AND name = 'Veg Room Grow Light' AND deleted_at IS NULL
    LIMIT 1;

    SELECT id INTO v_act_flower
    FROM gr33ncore.actuators
    WHERE farm_id = p_farm_id AND name = 'Flower Room Grow Light' AND deleted_at IS NULL
    LIMIT 1;

    -- ── Veg Room: 18/6 lighting_program ──────────────────────────────────────
    IF v_veg_zone IS NOT NULL AND v_act_veg IS NOT NULL
       AND NOT EXISTS (
           SELECT 1 FROM gr33ncore.lighting_programs
           WHERE farm_id = p_farm_id AND name = 'Veg Room 18/6 (Bootstrap)'
       ) THEN

        INSERT INTO gr33ncore.lighting_programs
            (farm_id, zone_id, actuator_id,
             name, description,
             on_hours, off_hours, lights_on_at, timezone,
             schedule_on_id, schedule_off_id,
             is_active, metadata)
        VALUES
            (p_farm_id, v_veg_zone, v_act_veg,
             'Veg Room 18/6 (Bootstrap)',
             'Standard vegetative photoperiod — 18 h on / 6 h off, lights on at 06:00.',
             18, 6, '06:00', p_tz,
             NULL, NULL,
             TRUE,
             jsonb_build_object('preset_key', 'veg_18_6', 'source', 'bootstrap'))
        RETURNING id INTO v_prog_id;

        -- ON schedule: 0 6 * * *
        INSERT INTO gr33ncore.schedules
            (farm_id, name, description, schedule_type,
             cron_expression, timezone, is_active, meta_data, preconditions)
        VALUES
            (p_farm_id,
             'LP-' || v_prog_id::TEXT || ' ON: Veg Room 18/6 (Bootstrap)',
             'Auto-generated ON schedule for lighting program ' || v_prog_id::TEXT,
             'lighting',
             '0 6 * * *',
             p_tz,
             TRUE,
             jsonb_build_object('lighting_program_id', v_prog_id, 'side', 'on'),
             '[]'::jsonb)
        RETURNING id INTO v_sch_on_id;

        -- OFF schedule: 0 0 * * * (18h after 06:00 = midnight)
        INSERT INTO gr33ncore.schedules
            (farm_id, name, description, schedule_type,
             cron_expression, timezone, is_active, meta_data, preconditions)
        VALUES
            (p_farm_id,
             'LP-' || v_prog_id::TEXT || ' OFF: Veg Room 18/6 (Bootstrap)',
             'Auto-generated OFF schedule for lighting program ' || v_prog_id::TEXT,
             'lighting',
             '0 0 * * *',
             p_tz,
             TRUE,
             jsonb_build_object('lighting_program_id', v_prog_id, 'side', 'off'),
             '[]'::jsonb)
        RETURNING id INTO v_sch_off_id;

        -- Link schedules back to the program
        UPDATE gr33ncore.lighting_programs
        SET schedule_on_id  = v_sch_on_id,
            schedule_off_id = v_sch_off_id
        WHERE id = v_prog_id;

        -- Executable actions
        INSERT INTO gr33ncore.executable_actions
            (schedule_id, execution_order, action_type,
             target_actuator_id, action_command, action_parameters)
        VALUES
            (v_sch_on_id,  0, 'control_actuator', v_act_veg, 'on',  '{}'::jsonb),
            (v_sch_off_id, 0, 'control_actuator', v_act_veg, 'off', '{}'::jsonb);
    END IF;

    -- ── Flower Room: 12/12 lighting_program ──────────────────────────────────
    IF v_flower_zone IS NOT NULL AND v_act_flower IS NOT NULL
       AND NOT EXISTS (
           SELECT 1 FROM gr33ncore.lighting_programs
           WHERE farm_id = p_farm_id AND name = 'Flower Room 12/12 (Bootstrap)'
       ) THEN
        ...
        INSERT INTO gr33ncore.lighting_programs
            (farm_id, zone_id, actuator_id,
             ...
             is_active, metadata)
        VALUES
            (p_farm_id, v_flower_zone, v_act_flower,
             'Flower Room 12/12 (Bootstrap)',
             'Standard flowering photoperiod — 12 h on / 12 h off, lights on at 06:00.',
             12, 12, '06:00', p_tz,
             NULL, NULL,
             TRUE,
             jsonb_build_object('preset_key', 'flower_12_12', 'source', 'bootstrap'))
        RETURNING id INTO v_prog_id;
        ...
        VALUES
            (p_farm_id,
             'LP-' || v_prog_id::TEXT || ' ON: Flower Room 12/12 (Bootstrap)',
             'Auto-generated ON schedule for lighting program ' || v_prog_id::TEXT,
             'lighting',
             '0 6 * * *',
             p_tz,
        ...
        -- OFF schedule: 0 18 * * * (12h after 06:00 = 18:00)
        INSERT INTO gr33ncore.schedules
            (farm_id, name, description, schedule_type,
             cron_expression, timezone, is_active, meta_data, preconditions)
        VALUES
            (p_farm_id,
             'LP-' || v_prog_id::TEXT || ' OFF: Flower Room 12/12 (Bootstrap)',
             'Auto-generated OFF schedule for lighting program ' || v_prog_id::TEXT,
             'lighting',
             '0 18 * * *',
             p_tz,
        ...
        INSERT INTO gr33ncore.executable_actions
            (schedule_id, execution_order, action_type,
             target_actuator_id, action_command, action_parameters)
        VALUES
            (v_sch_on_id,  0, 'control_actuator', v_act_flower, 'on',  '{}'::jsonb),
            (v_sch_off_id, 0, 'control_actuator', v_act_flower, 'off', '{}'::jsonb);
    END IF;
...
-- =============================================================================
-- Patch _bootstrap_jadam_indoor_photoperiod_v1 to call the lighting block.
--
-- We redefine the function, appending PERFORM _bootstrap_phase35_lighting_block
-- at the end. Everything before that line is identical to the 20260429 version.
-- =============================================================================
