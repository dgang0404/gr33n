-- Phase 124 — fix gr33ncore._bootstrap_jadam_indoor_photoperiod_v1: it still
-- inserted into crop_cycles.strain_or_variety, a column renamed to
-- batch_label by Phase 93 (20260620_phase93_crop_cycles_batch_label.sql).
-- Since Postgres doesn't validate plpgsql function bodies at CREATE time,
-- this went undetected until the function actually ran — which broke every
-- new-farm bootstrap (POST /farms with a template, and
-- POST /farms/{id}/bootstrap-template) with:
--   ERROR: column "strain_or_variety" of relation "crop_cycles" does not exist
-- Body otherwise unchanged from the live definition (last touched by
-- 20260603_phase35_oc35a_bootstrap_lighting_programs.sql).
