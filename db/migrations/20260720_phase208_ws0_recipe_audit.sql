-- Phase 208 WS0 — Natural farming recipe audit (Cho 2016 / KNF honesty)
-- Fixes JMS dilution drift (1:500 → 1:10 soil / 1:20 foliar), input prep errors,
-- KNF reference_source labels, adds FAA. See docs/field-guides/procedures/recipe-audit-log.md

-- ── Application recipes (all farms) ────────────────────────────────────────
UPDATE gr33nnaturalfarming.application_recipes
SET dilution_ratio = '1:10 (JMS:water)',
    instructions = 'Dilute 1:10 (1 part JMS to 10 parts water). Apply 2-4L per sqm of root zone. Morning or evening.'
WHERE name = 'JMS Soil Drench' AND deleted_at IS NULL
  AND dilution_ratio LIKE '%1:500%';

UPDATE gr33nnaturalfarming.application_recipes
SET dilution_ratio = '1:20 (JMS:water) + JWA',
    instructions = 'Dilute 1:20. Add JWA for leaf coverage. Spray upper and lower leaf surfaces to runoff. Early morning.'
WHERE name = 'JMS Foliar Spray' AND deleted_at IS NULL
  AND dilution_ratio LIKE '%1:500%';

UPDATE gr33nnaturalfarming.application_recipes
SET dilution_ratio = 'JLF 1:20 + JMS 1:10 in same water',
    instructions = 'Fill tank. Add JLF 1:20, then JMS 1:10. Apply same day.'
WHERE name = 'JLF and JMS Combined Drench' AND deleted_at IS NULL
  AND dilution_ratio LIKE '%JMS 1:500%';

UPDATE gr33nnaturalfarming.application_recipes
SET instructions = 'Strain JLF through cloth. Start 1:100 if unsure; dilute 1:20 when tested. Apply 2-4L per sqm to root zone.'
WHERE name = 'JLF General Soil Drench' AND deleted_at IS NULL
  AND instructions NOT LIKE '%1:100%';

UPDATE gr33nnaturalfarming.application_recipes
SET dilution_ratio = '0.5-2 L JS concentrate per 500 L water + JWA 1:500',
...
WHERE name = 'JS Fungicide Spray' AND deleted_at IS NULL
  AND dilution_ratio LIKE '%0.5% JS%';

-- ── Recipe input components ────────────────────────────────────────────────
UPDATE gr33nnaturalfarming.recipe_input_components c
SET part_value = 2.0, notes = 'JMS at 1:10 relative to 1:20 JLF base'
FROM gr33nnaturalfarming.application_recipes r, gr33nnaturalfarming.input_definitions d
WHERE c.application_recipe_id = r.id AND c.input_definition_id = d.id
  AND r.name = 'JLF and JMS Combined Drench' AND r.deleted_at IS NULL
  AND d.name LIKE 'JMS%' AND c.notes LIKE '%1:500%';

UPDATE gr33nnaturalfarming.recipe_input_components c
SET notes = '1 part JMS to 10 parts water'
FROM gr33nnaturalfarming.application_recipes r, gr33nnaturalfarming.input_definitions d
WHERE c.application_recipe_id = r.id AND c.input_definition_id = d.id
  AND r.name = 'JMS Soil Drench' AND r.deleted_at IS NULL
  AND d.name LIKE 'JMS%' AND c.notes LIKE '%500%';

UPDATE gr33nnaturalfarming.recipe_input_components c
SET notes = '1 part JMS to 20 parts water'
FROM gr33nnaturalfarming.application_recipes r, gr33nnaturalfarming.input_definitions d
WHERE c.application_recipe_id = r.id AND c.input_definition_id = d.id
  AND r.name = 'JMS Foliar Spray' AND r.deleted_at IS NULL
  AND d.name LIKE 'JMS%' AND c.notes LIKE '%500%';

UPDATE gr33nnaturalfarming.recipe_input_components c
SET notes = '0.5-2 L JS concentrate per 500 L water'
FROM gr33nnaturalfarming.application_recipes r, gr33nnaturalfarming.input_definitions d
WHERE c.application_recipe_id = r.id AND c.input_definition_id = d.id
  AND r.name = 'JS Fungicide Spray' AND r.deleted_at IS NULL
  AND d.name LIKE 'JS%' AND c.notes LIKE '%0.5% sulfur%';

-- ── Input definitions ──────────────────────────────────────────────────────
UPDATE gr33nnaturalfarming.input_definitions
SET reference_source = 'KNF (Cho Han-kyu); often used with JADAM'
WHERE name IN (
    'LAB (Lactic Acid Bacteria Serum)', 'FPJ (Fermented Plant Juice)',
    'FFJ (Fermented Fruit Juice)', 'OHN (Oriental Herbal Nutrient)',
    'WCA (Water-Soluble Calcium)', 'WCS (Water-Soluble Calcium Phosphate)'
...
WHERE name = 'JLF Spring (Nettle and Comfrey)' AND deleted_at IS NULL
  AND description LIKE '%nitrogen-fixing%';

UPDATE gr33nnaturalfarming.input_definitions
SET typical_ingredients = 'Fresh herbs or Jerusalem artichoke, non-chlorinated water',
    preparation_summary = 'Boil 1 kg fresh plant in mesh bag in 4-5 L water 4-5 hours. Strain finely. Use fresh concentrate.'
WHERE name = 'JHS (JADAM Herbal Solution)' AND deleted_at IS NULL
  AND preparation_summary LIKE '%Simmer%';

UPDATE gr33nnaturalfarming.input_definitions
SET preparation_summary = 'Boil potato, suspend in mesh bag in 10-20L water with leaf mold and pinch of salt. Ferment 24-72 h to peak foam; use within 6-12 h of peak.',
    storage_guidelines = 'Use at peak foam — within 6-12 h of peak. Cool, shaded.'
WHERE name LIKE 'JMS (JADAM Microbial Solution)%' AND deleted_at IS NULL;

UPDATE gr33nnaturalfarming.input_definitions
SET description = REPLACE(description, 'Dilution 1:20 general', 'Start 1:100 if unsure; experienced use 1:20')
WHERE name LIKE 'JLF General%' AND deleted_at IS NULL;

UPDATE gr33nnaturalfarming.input_definitions
SET typical_ingredients = 'Ripe/overripe fruits, brown sugar (1:1 by weight)'
WHERE name LIKE 'FFJ%' AND deleted_at IS NULL
  AND typical_ingredients LIKE '%water%';

-- FAA — add to farms that already have JADAM seed rows
...
FROM gr33nnaturalfarming.input_definitions d
WHERE d.name LIKE 'JMS (JADAM Microbial Solution)%' AND d.deleted_at IS NULL
  AND NOT EXISTS (
      SELECT 1 FROM gr33nnaturalfarming.input_definitions f
      WHERE f.farm_id = d.farm_id AND f.name = 'FAA (Fish Amino Acid)' AND f.deleted_at IS NULL
  );

-- Demo copy: task + mixing history strings
UPDATE gr33ncore.tasks
SET description = REPLACE(description, 'JMS 1:500', 'JMS 1:10')
WHERE description LIKE '%JMS 1:500%' AND deleted_at IS NULL;

UPDATE gr33nfertigation.mixing_event_components
SET dilution_ratio = '1:10 in tank',
    volume_added_ml = CASE WHEN volume_added_ml <= 1000 THEN volume_added_ml * 50 ELSE volume_added_ml END,
    notes = REPLACE(notes, '1:500', '1:10')
WHERE dilution_ratio = '1:500 in tank';

-- ── Bootstrap template (new farms) ─────────────────────────────────────────
...
BEGIN
    ...
    -- Legacy orphan lighting schedules (inactive). Kept for backward compat;
    -- the lighting_programs added below at the end of this function are the
    -- canonical source for new farms. Existing farms that ran an older version
    -- of this bootstrap may have these without lighting_programs — they can
    -- upgrade via POST /farms/{id}/lighting-programs/from-preset.
    ...
    FROM (
        VALUES
            ('JMS (JADAM Microbial Solution)', 'microbial_inoculant',
             ...
             'Ferment 24-72 h to peak foam; use within 6-12 h of peak.', 'Use at peak foam.', 'Non-chlorinated water.', 'JADAM Organic Farming, Youngsang Cho, 2016'),
            ...
            ('LAB (Lactic Acid Bacteria Serum)', 'microbial_inoculant',
             ...
             'Rice wash, milk',
             'Ferment; extract serum.', 'Refrigerate preserved.', 'Use golden layer only.', 'KNF (Cho Han-kyu); often used with JADAM'),
            ('FFJ (Fermented Fruit Juice)', 'fermented_plant_juice',
             'Sweet ferment for flowering support.',
             'Ripe fruit, brown sugar (1:1 by weight)',
             'Chop fruit, mix 1:1 with sugar. Ferment 7 days; strain.', 'Refrigerate.', 'Avoid over-concentration.', 'KNF (Cho Han-kyu); often used with JADAM'),
            ('WCA (Water-Soluble Calcium)', 'water_soluble_nutrient',
             'Calcium from eggshell vinegar extract.',
             'Eggshells, vinegar',
             'Extract; dilute.', 'Store labeled.', 'Acid — eye protection.', 'KNF (Cho Han-kyu); often used with JADAM')
    ...
    FROM (
        VALUES
            ('JMS Soil Drench', 'Base soil microbe inoculant.', 'soil_drench', '1:10 (JMS:water)',
            ...
            ('JLF General Soil Drench', 'Primary JLF soil fertility.', 'soil_drench', '1:20 (JLF:water)',
             ...
             ARRAY['All crops']::text[], ARRAY['All stages']::text[]),
            ('JLF and JMS Combined Drench', 'Nutrients + microbes in one pass.', 'soil_drench', 'JLF 1:20 + JMS 1:10',
    ...
    INSERT INTO gr33nnaturalfarming.recipe_input_components
        (application_recipe_id, input_definition_id, part_value, part_unit_id, notes)
    SELECT r.id, d.id, 2.0, u.id, 'JMS at 1:10 relative to JLF base'
    FROM gr33nnaturalfarming.application_recipes r
    CROSS JOIN gr33nnaturalfarming.input_definitions d
    CROSS JOIN gr33ncore.units u
    WHERE r.farm_id = p_farm_id AND d.farm_id = p_farm_id AND u.name = 'decimal_fraction'
    ...
    IF r_veg IS NOT NULL AND NOT EXISTS (
        ...
        IF i_jms IS NOT NULL THEN
            INSERT INTO gr33nfertigation.mixing_event_components
                (mixing_event_id, input_definition_id, input_batch_id, volume_added_ml, dilution_ratio, notes)
            VALUES (mix_veg, i_jms, b_jms, 30000.000, '1:10 in tank', 'Draw from Inventory batch TPL-JMS-001');
        END IF;
    END IF;
    ...
    -- Crop cycles (Phase 124: strain_or_variety -> batch_label, Phase 93 rename)
    ...
    -- ── Phase 35 OC-35A: lighting_programs (devices + actuators + programs) ──
    PERFORM gr33ncore._bootstrap_phase35_lighting_block(p_farm_id, p_tz);

END;
$function$;

-- ponytail: WS0 closure — fail migration if JMS soil drench still wrong
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM gr33nnaturalfarming.application_recipes
        WHERE name = 'JMS Soil Drench' AND deleted_at IS NULL AND dilution_ratio LIKE '%1:500%'
    ) THEN
        RAISE EXCEPTION 'phase208 WS0: JMS Soil Drench still at 1:500';
    END IF;
END $$;
