...
Use the live farm snapshot in Guardian for heartbeat and command state. Describe what you see — Guardian labels **operator-stated** facts separately from measurements.$fg_field_troubleshooting$, 4, TRUE, 5),
...
**Common mistakes:** Lockout from pH drift (target 5.8–6.2 in hydro); running flower EC in veg (salt stress); skipping flush before harvest.$fg_crop_cannabis_nutrition$, 4, TRUE, 6),
...
Guardian compares your readings to the tomato profile stage row assigned to the plant.$fg_crop_tomato_nutrition$, 4, TRUE, 7),
...
Use the phalaenopsis built-in profile for Guardian targets; narrative here explains *why* the numbers are low.$fg_crop_orchid_care$, 4, TRUE, 8),
...
Guardian uses the pepper built-in profile stage row — not tomato peak EC by default.$fg_crop_pepper_nutrition$, 4, TRUE, 9),
...
Assign the lettuce profile in Start grow or Plants so Guardian cites structured mS/cm targets.$fg_crop_lettuce_nutrition$, 4, TRUE, 10),
...
Use the basil profile for Guardian feed targets; it is not interchangeable with cilantro or lettuce bands.$fg_crop_basil_nutrition$, 4, TRUE, 11),
...
Guardian cites the strawberry profile; June-bearing vs day-neutral genetics may need a cloned farm override.$fg_crop_strawberry_nutrition$, 4, TRUE, 12),
...
Guardian cites mS/cm targets from the eggplant built-in profile; do not assume tomato numbers apply unchanged.$fg_crop_eggplant_nutrition$, 4, TRUE, 13),
...
Use the cucumber profile stage row for Guardian targets — never guess EC from cannabis or lettuce baselines.$fg_crop_cucumber_nutrition$, 4, TRUE, 14),
...
Guardian compares live readings to the kale profile assigned to the plant.$fg_crop_kale_nutrition$, 4, TRUE, 15),
...
Harvest on schedule; do not treat spinach like basil heat tolerance.$fg_crop_spinach_nutrition$, 4, TRUE, 16),
...
Guardian cites the cilantro profile; "coriander" is the same crop for lookup purposes.$fg_crop_cilantro_nutrition$, 4, TRUE, 17),
...
Targets are per tray turnover, not months-long veg stages.$fg_crop_microgreens_nutrition$, 4, TRUE, 18),
...
Guardian cites mS/cm from the zucchini built-in profile.$fg_crop_zucchini_nutrition$, 4, TRUE, 19),
...
Use the green bean profile for structured targets — not tomato peak EC.$fg_crop_green_bean_nutrition$, 4, TRUE, 20),
...
Continuous harvest keeps plants vegetative; EC creep causes tip burn on tender leaves.$fg_crop_mint_nutrition$, 4, TRUE, 21),
...
Biennial plants may bolt in year two if rooms run hot; succession sow for steady supply.$fg_crop_parsley_nutrition$, 4, TRUE, 22),
...
Monitor runoff pH weekly; peat/coco mixes buffer differently than pure rockwool.$fg_crop_blueberry_nutrition$, 4, TRUE, 23),
...
For CBD/flower hemp genetics, clone and customize a farm profile from your agronomist.$fg_crop_hemp_nutrition$, 4, TRUE, 24),
...
Harvest on schedule; extended warm veg wastes bench space.$fg_crop_broccoli_nutrition$, 4, TRUE, 25),
...
Guardian compares live readings to the melon profile stage row.$fg_crop_melon_nutrition$, 4, TRUE, 26),
...
Harvest young leaves; succession plant for steady supply.$fg_crop_arugula_nutrition$, 4, TRUE, 27),
...
Guardian compares your readings to the rose profile stage row assigned to the plant.$fg_crop_rose_care$, 4, TRUE, 28),
...
Guardian cites the sunflower profile for feed targets matched to your assigned growth stage.$fg_crop_sunflower_nutrition$, 4, TRUE, 29),
...
Use the hops profile in Guardian; do not copy indoor cannabis flower bands onto bines.$fg_crop_hops_nutrition$, 4, TRUE, 30),
...
Guardian compares live readings to the succulent profile assigned to the plant.$fg_crop_succulent_care$, 4, TRUE, 31),
...
Use the san pedro built-in profile for Guardian targets during each season row.$fg_crop_san_pedro_care$, 4, TRUE, 32),
...
Guardian cites the species profile assigned to each plant, not a single generic houseplant target.$fg_crop_houseplant_care$, 4, TRUE, 33),
...
Guardian compares your readings to the chard profile stage row assigned to the plant.$fg_crop_chard_nutrition$, 4, TRUE, 34),
...
Use the bok choy profile in Guardian; summer rooms may need lower EC and shorter cycles.$fg_crop_bok_choy_nutrition$, 4, TRUE, 35),
...
Guardian cites the radish profile for feed targets matched to your short cycle stage.$fg_crop_radish_nutrition$, 4, TRUE, 36),
...
Guardian compares live readings to the thyme profile assigned to the plant.$fg_crop_thyme_nutrition$, 4, TRUE, 37),
...
Use the oregano profile in Guardian; do not swap in basil EC bands.$fg_crop_oregano_nutrition$, 4, TRUE, 38),
...
Guardian cites the rosemary profile for targets; fix drainage before raising EC.$fg_crop_rosemary_nutrition$, 4, TRUE, 39),
...
Guardian compares your readings to the lavender profile stage row assigned to the plant.$fg_crop_lavender_nutrition$, 4, TRUE, 40),
...
Guardian cites the chrysanthemum profile for stage targets tied to your photoperiod schedule.$fg_crop_chrysanthemum_care$, 4, TRUE, 41),
...
Fruit on bench-scale trees typically **years 3–5+**. Train central leader early; increase light (DLI) as wood hardens.$fg_crop_apple_nursery$, 4, TRUE, 42),
...
Fruit in pots often **year 2–4** depending on cultivar and graft. Reduce feed slightly if new flush is weak after repotting.$fg_crop_citrus_nursery$, 4, TRUE, 43),
...
Breba vs main crop timing is **cultivar-dependent**. If trees go dormant, cut feed and let substrate dry more between irrigations.$fg_crop_fig_container$, 4, TRUE, 44),
...
Nectarine shares the same bench targets as peach. Fruiting in containers often **years 2–4**.$fg_crop_peach_nursery$, 4, TRUE, 45),
...
Rain at harvest causes **cracking** outdoors; in greenhouse, keep VPD stable during fruit swell if you push early crops.$fg_crop_cherry_nursery$, 4, TRUE, 46),
...
First meaningful crop often **year 2–3** on greenhouse vines. Alias terms: grapevine, grape vine.$fg_crop_grape_vine$, 4, TRUE, 47),
...
Juvenile phase is long; fruit in pots may take **several years**. Graft union must stay above the substrate line.$fg_crop_avocado_nursery$, 4, TRUE, 48),
...
Train scaffolds early in containers; fruiting years similar to apple on bench scale.$fg_crop_pear_nursery$, 4, TRUE, 49),
...
Fruit in containers typically **years 3–5**. Thin heavy sets on small trees to avoid branch break.$fg_crop_plum_nursery$, 4, TRUE, 50),
...
Juvenile mangoes in pots may fruit **years 3–5+**. Never ship cold-stressed liners into warm zones without acclimation.$fg_crop_mango_nursery$, 4, TRUE, 51),
...
Direct operators to husbandry / substrate workflows when available. Do not map shiitake or other fungi to cannabis or tomato nutrient curves.$fg_crop_unsupported_mushroom$, 4, TRUE, 53),
...
If an operator wants indoor hydro only, suggest cloning from **lettuce** (fast leafy baseline) or **tomato** (fruiting hydro) and adjusting manually — do not state fake EC targets for tubers.$fg_crop_unsupported_field_roots$, 4, TRUE, 54)
