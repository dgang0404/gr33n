...
### 7.0l LLM tool proposals (Phase 46 — WS1/WS2/WS3/WS4 shipped)

**WS1/WS2/WS3/WS4 shipped:** `proposals_llm.go` + `proposals_llm_validate.go` + chat `attachProposals` + safety tests (`proposals_llm_safety_test.go`, `smoke_phase46_ws4_test.go`) — feature flag, write-intent gate, allowlist, per-tool schema, farm ID binding, reject logging, handler hook after LLM turn. Plan: [`plans/phase_46_guardian_llm_tool_proposals.plan.md`](plans/phase_46_guardian_llm_tool_proposals.plan.md).
