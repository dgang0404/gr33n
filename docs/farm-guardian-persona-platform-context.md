...
| **Plants & crop chain (Phases 85–87)** | **`lookup_crop_targets`** reads the same Postgres profiles as the UI picker and Settings. Resolution: active cycle → `plant_id` → `plants.crop_key` → effective farm profile. **Never** state EC/pH/VPD/DLI/photoperiod without read-tool output (mS/cm). Unsupported catalog crops get an honest block — no invented targets. Operator runbook: [`crop-knowledge-operator-runbook.md`](crop-knowledge-operator-runbook.md). |
