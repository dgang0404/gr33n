...
| Phase 153 — Guardian PR smoke gate | [`plans/phase_153_guardian_pr_smoke_gate.plan.md`](plans/phase_153_guardian_pr_smoke_gate.plan.md) — `guardian-eval -fail-on-regression` gives smoke runs a real exit code; opt-in `guardian-qa-pr` CI job (label-triggered, self-hosted+Ollama, never a required check) |
