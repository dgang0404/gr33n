...
All API calls go over plain HTTP(S); there is no long-lived socket. Authentication is a pre-shared **API key** sent as the `X-Api-Key` header on every request. The API validates it with `requireAPIKey` middleware (`cmd/api/routes.go`).
...
- **API key rotation:** redeploy both the API (`PI_API_KEY` env var) and each Pi's `config.yaml` / systemd unit env file, then restart both. The Pi will get 401s and requeue readings locally until the key matches again — no data is lost.
