...
> Read `docs/plans/archive/phase_51_pi_config_sync.plan.md` and `docs/plans/archive/phase_50_hardware_wiring_visibility.plan.md`. Implement one workstream (WS1–WS6). Phase 50 WS1 wiring model must exist first. Do not change the `PI_API_KEY` auth contract. Existing full-YAML config.yaml deployments must keep working unchanged.
