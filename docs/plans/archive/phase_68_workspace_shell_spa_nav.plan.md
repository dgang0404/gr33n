...
> Read `docs/plans/archive/phase_68_workspace_shell_spa_nav.plan.md`. Implement one workstream (WS1–WS6). UI-only: never delete a route — redirect it into `workspace?tab=`. Tabs must be `?tab=` deep-linkable. Honor prefers-reduced-motion. Do not merge tab content yet (that's Phases 69–72).
