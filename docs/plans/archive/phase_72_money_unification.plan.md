...
> Read `docs/plans/archive/phase_72_money_unification.plan.md`. UI-only. Merge MoneyHub + Costs + SuppliesHub (+ Inventory) into the `/money` workspace as tabs (This month → Ledger → Supplies & costs). De-orphan /costs and /inventory. Redirect old routes. Keep restock/receipts inline (no new Guardian write tools).
