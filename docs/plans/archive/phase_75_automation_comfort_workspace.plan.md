...
> Read `docs/plans/archive/phase_75_automation_comfort_workspace.plan.md`. UI-only. Wrap ComfortTargetsHub + Schedules + Automation + Setpoints in a WorkspaceShell at `/comfort-targets` with four tabs. Redirect legacy Advanced routes. Remove Advanced nav group. Zone Climate tab stays inline.
