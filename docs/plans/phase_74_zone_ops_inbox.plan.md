...
> Read `docs/plans/phase_74_zone_ops_inbox.plan.md`. UI-only. Absorb Tasks, Alerts, and Plants into the zone SPA (Ops + Plants tabs on ZoneDetail; Strains under Zones workspace). Keep Today as farm-wide triage. Remove Tasks/Alerts/Plants from sidebar. Redirect legacy routes; never delete paths.
