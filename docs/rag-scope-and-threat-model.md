...
## 9. Education layer vs RAG; operational logs (Phase 26 WS3)

This section fixes the **product boundary** between (a) teaching operators how gr33n works, (b) retrieving **farm database text**, and (c) **application / access logs**. It complements §4–§6 (what tables may be embedded) and **[operator-logging-runbook.md](operator-logging-runbook.md)** (how stdout logs are captured).

### 9.1 Static education (not automatic RAG)

| Channel | Role | Default RAG relationship |
|---------|------|---------------------------|
| **Operator Guide** (`/operator-guide`), **[operator-tour.md](operator-tour.md)**, **[workflow-guide.md](workflow-guide.md)** glossary | Explain schedules vs rules, setpoints vs readings, navigation order | **Not** ingested into `rag_embedding_chunks` unless product ships an explicit **“help library”** ingest domain later. Teaching copy stays versioned in repo/UI. |
| **HelpTip** copy in Vue | Short inline cues | Same — curated UI strings, not DB chunks. |

Operators learn concepts from static docs; **Knowledge** answers questions using **indexed farm text** from approved SQL domains (§4.2 / §6).

### 9.2 Farm-grounded RAG (in scope today)

- **Sources:** Only rows processed by **`rag-ingest`** from checklist-approved tables, after **`internal/rag/sanitize`** rules.
- **Storage:** `gr33ncore.rag_embedding_chunks` — farm-isolated (§3).
- **API:** `/farms/{id}/rag/search` and `/rag/answer` — JWT + farm membership; no alternate “log search” endpoint.

### 9.3 Operational and HTTP logs — **out of scope for ingestion**

| Source | Why **not** default embed fodder |
|--------|-----------------------------------|
| Structured **`request`** / **`auth_rejected`** / worker **`slog`** lines | Paths, ids, timing — overlap **security-sensitive** patterns and **noise** versus curated domain text (§2.2 “Logs”). |
| Docker **`docker logs`**, Loki/Grafana stacks | Aggregation for **ops/debug**, not Knowledge retrieval — see **[operator-logging-runbook.md](operator-logging-runbook.md)**. |

**Rule:** Do **not** point **`rag-ingest`** at log files, Loki exports, or stderr captures without a **new product decision**, explicit **allowlist**, **redaction**, and **privacy review**.

### 9.4 Future: summarized incidents in Knowledge (hypothetical)

If the product ever wants “what broke Tuesday?” inside **Knowledge**, treat it as a **new domain**, not raw logs:

1. **Allowlisted** event types (e.g. automation failure summaries already stored as **`automation_runs.message`** may already be embeddable per §4.2 — distinct from shipping raw access logs).
2. Optional **curated** summary rows (farm-scoped) generated by a trusted pipeline with **retention** and **consent** documented.
3. **Never** verbatim production HTTP access logs unless separately audited for PII and tenant isolation.
...
*This file is the authoritative Phase 24 scope, threat-model, and WS2 storage record; Phase 26 WS3 adds §9 (education vs RAG vs logs).*
