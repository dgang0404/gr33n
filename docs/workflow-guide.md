...
### 4b. Operator UX roadmap (39–43 shipped; 44 next)

**Phase 39 — shipped.** FIFO **`device_commands`** queue, cloud **`MixPlan`**, Pi **`mix_batch`** + **pulse**, reservoir base EC, Zone Water preview. Canonical plan: [`plans/archive/phase_39_edge_fertigation_execution.plan.md`](plans/archive/phase_39_edge_fertigation_execution.plan.md).

**Phase 40 — shipped.** Zone cockpit on **Overview / Water / Light / Climate** — Today strip, inline comfort targets, zone alerts, What runs when, Water grow story, zone tasks, Ask gr33n starters. Plan: [`plans/archive/phase_40_unified_farmer_ux_zone_cockpit.plan.md`](plans/archive/phase_40_unified_farmer_ux_zone_cockpit.plan.md).
...
| **[41 — Farm hub](plans/archive/phase_41_farm_hub_coherence.plan.md)** ✅ | **Dashboard** morning chips; **Fertigation** honors `?zone_id=`; **Tasks / Schedules / Alerts** keep zone context; **why-empty** hints | Closes the “six sidebar hops” problem when the zone cockpit is done but farm-wide pages still feel disconnected. Walkthrough: [operator-tour §3b](operator-tour.md#3b-farm-hub--morning-path-phase-41). |
| **[47 — Feeding & water](plans/archive/phase_47_feeding_water_plain_language.plan.md)** ✅ | **`/feeding`** hub + zone **Water** feeding plan — room-first, no six-tab console | Daily grow watering job; Advanced **Feeding (technical)** for power users. Walkthrough: [operator-tour §7b](operator-tour.md#7b-feeding--water-for-this-room-phase-47). |
| **[42 — Comfort & automation](plans/archive/phase_42_comfort_targets_automation_plain_language.plan.md)** ✅ | **Targets** nav; comfort bands; plain schedules/rules | Operators set bands without opening Setpoints table first. Walkthrough: [operator-tour §5c](operator-tour.md#5c-comfort-bands--what-runs-when-phase-42--shipped). |
| **[43 — Operations hub](plans/archive/phase_43_operations_stock_feeding_finance.plan.md)** ✅ | **Operations → Supplies / Feeding (details) / Money** — restock, farm-wide feeding admin, receipts | Replaces schema-first Inventory/Fertigation/Costs in farmer nav; Advanced keeps full editors. Walkthrough: [operator-tour §7](operator-tour.md#7-supplies-feeding--money-phase-43). |
| **[44–45 — Farmer UX completion](plans/farmer_ux_roadmap_40_plus.plan.md)** | Setup/Pi wizards (44), sit-in polish (45) | Full non-technical farmer v1 — UI composes existing APIs; schema stable. |
| **Guardian PR UX (doc gate)** | [guardian-change-requests-guide.md](guardian-change-requests-guide.md) · [guardian_pr_ux_through_farmer_phases.plan.md](plans/archive/guardian_pr_ux_through_farmer_phases.plan.md) | How proposals are triggered, starters vs Confirm, inbox — read before Phase 40 zone starters. |
...
**UX & degraded responses (Phase 25 WS6):** The Knowledge UI treats **503** / **502** / **429** / **504** as *configuration or upstream* issues (amber styling): **503** usually means missing **`EMBEDDING_API_KEY`** (search) or **`LLM_BASE_URL` + `LLM_MODEL`** (answer), matching JSON errors from **`internal/handler/rag`**. Configure the API host via **[`INSTALL.md`](../INSTALL.md)** (compose / env) and **[`example-env.md`](example-env.md)**. Where **`rag_embedding_chunks`** sits in the schema graph: **[`schema-erd-text.md`](schema-erd-text.md)** §2; broader overview: **[`database-schema-overview.md`](database-schema-overview.md)**. Phase tracker: **[`phase_25_rag_operations_and_expansion.plan.md`](plans/archive/phase_25_rag_operations_and_expansion.plan.md)**.
