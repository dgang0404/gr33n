...
type GetRecentChatBudgetWarningForUserParams struct {
	RecipientUserID pgtype.UUID `db:"recipient_user_id" json:"recipient_user_id"`
...
func (q *Queries) GetRecentChatBudgetWarningForUser(ctx context.Context, arg GetRecentChatBudgetWarningForUserParams) (int64, error) {
	row := q.db.QueryRow(ctx, getRecentChatBudgetWarningForUser, arg.RecipientUserID, arg.CreatedAt)
	var id int64
	err := row.Scan(&id)
	return id, err
}
...
type ListRecentUnreadAlertsByFarmParams struct {
	FarmID int64 `db:"farm_id" json:"farm_id"`
	Limit  int32 `db:"limit" json:"limit"`
}

type ListRecentUnreadAlertsByFarmRow struct {
	ID                        int64                              `db:"id" json:"id"`
	Severity                  *Gr33ncoreNotificationPriorityEnum `db:"severity" json:"severity"`
	SubjectRendered           *string                            `db:"subject_rendered" json:"subject_rendered"`
	MessageTextRendered       *string                            `db:"message_text_rendered" json:"message_text_rendered"`
...
func (q *Queries) ListRecentUnreadAlertsByFarm(ctx context.Context, arg ListRecentUnreadAlertsByFarmParams) ([]ListRecentUnreadAlertsByFarmRow, error) {
	rows, err := q.db.Query(ctx, listRecentUnreadAlertsByFarm, arg.FarmID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListRecentUnreadAlertsByFarmRow{}
	for rows.Next() {
		var i ListRecentUnreadAlertsByFarmRow
		if err := rows.Scan(
			&i.ID,
			&i.Severity,
			&i.SubjectRendered,
