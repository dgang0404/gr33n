...
func TestAttachProposals_SkipsWithoutUserOrFarm(t *testing.T) {
	...
	resp := &postResponse{}
	h.attachProposals(context.Background(), 1, false, uuid.Nil, uuid.Nil, "Set feed to 0.3 L", "{}", farmguardian.Snapshot{}, nil, resp)
	if len(resp.Proposals) != 0 {
		t.Fatalf("expected no proposals without user, got %+v", resp.Proposals)
	}
	h.attachProposals(context.Background(), 0, true, uuid.New(), uuid.Nil, "Set feed to 0.3 L", "{}", farmguardian.Snapshot{}, nil, resp)
	if len(resp.Proposals) != 0 {
...
func TestAttachProposals_SkipsLLMWhenFlagOff(t *testing.T) {
	...
	h.attachProposals(ctx, 1, true, uuid.New(), uuid.Nil, "Update feed volume to 0.3 L", assistant, farmguardian.Snapshot{}, nil, resp)
	if len(resp.Proposals) != 0 {
