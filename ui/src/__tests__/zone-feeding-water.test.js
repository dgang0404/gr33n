...
describe('Phase 47 WS2 — zone Water primary surface', () => {
  ...
  it('shows feeding plan card and status line without fertigation as primary CTA', () => {
    ...
    expect(historyTo.path).toBe('/zones/3')
    expect(historyTo.query.tab).toBe('water')
    expect(historyTo.hash).toBe('#zone-feed-history')
    const suppliesTo = JSON.parse(wrapper.find('[data-test="zone-water-supplies-link"]').attributes('data-to'))
    expect(suppliesTo.path).toBe('/money')
    expect(suppliesTo.query.tab).toBe('supplies')
    ...
    expect(advancedTo.path).toBe('/zones/3')
    expect(advancedTo.hash).toBe('#zone-water-plan')
  })
